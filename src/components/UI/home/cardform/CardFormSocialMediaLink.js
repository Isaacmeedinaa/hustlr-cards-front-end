  ...
  return (
    <div className="card-form-social-media-input-group">
      ...
      {originalLink.url !== updatedLink.url ? (
        ...
        >
          {linkLoader.updatingLoader &&
          linkLoader.updatingLinkId === updatedLink.id ? (
      ...
      <button
        onClick={() => dispatch(deleteLink(updatedLink.id))}
        className="primary-color card-form-link-btn"
      >
        {linkLoader.deletingLoader &&
        linkLoader.deletingLinkId === updatedLink.id ? (
          <Loader type="TailSpin" color="#2ecc71" width={22} height={22} />
        ) : (
          "Delete"
        )}
